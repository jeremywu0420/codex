...
export function collectWireJunctionDots(graph: CircuitGraph) {
  const endpointCounts = new Map<string, { x: number; y: number; count: number; netId: string }>();
  ...
  const segments = graph.edges.flatMap((edge) =>
    pointsToSegments(pointsFromFlat(edge.points)).map((segment) => ({ ...segment, wireId: edge.wireId ?? edge.id, netId: edge.netId ?? "" })),
  );
  const pointIsOnSegmentInterior = (point: Point, segment: Segment) => {
    if (point.x === segment.from.x && point.y === segment.from.y) return false;
    if (point.x === segment.to.x && point.y === segment.to.y) return false;
  ...
  for (const edge of graph.edges) {
    for (const anchor of [edge.sourceAnchor, edge.targetAnchor]) {
      if (!anchor) continue;
      const key = `${edge.netId ?? ""}:${pointKey(anchor)}`;
      const existing = endpointCounts.get(key);
      endpointCounts.set(key, { x: anchor.x, y: anchor.y, netId: edge.netId ?? "", count: (existing?.count ?? 0) + 1 });
    }
  }
  for (const point of endpointCounts.values()) {
    if (point.count > 1) addVisibleDot(point);
    else if (point.netId && segments.some((segment) => segment.netId === point.netId && pointIsOnSegmentInterior(point, segment))) addVisibleDot(point);
  }
  return [...dots.values()];
}
