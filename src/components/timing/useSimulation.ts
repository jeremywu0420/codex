...
import { generateTimingData, parseInputSequence } from "../../logic/timing";
import type { TimingData, TimingStep } from "../../logic/timing";
...
export function useSimulation(input: UseSimulationInput) {
  ...
  const simulationResult = useMemo(() => {
    try {
      return {
        error: "",
        simulation: buildInteractiveSimulation(input),
      };
    } catch (error) {
      return {
  ...
  const maxStep = Math.max(0, (simulationResult.simulation?.cycles.length ?? 1) - 1);
  const selectedCycle = simulationResult.simulation?.cycles[currentStep] ?? null;
  ...
  useEffect(() => {
    if (!isAutoRunning || !simulationResult.simulation?.cycles.length) return undefined;
  ...
  }, [isAutoRunning, maxStep, simulationResult.simulation?.cycles.length, speedMs]);
